...
function VendorMembers() {
  ...
  const {
    ...
    fetchError,
    refetch,
  } = useGet("http://10.125.121.208:8080/car?requestStatus=pending", []);
    ...
    <div className="flex flex-col justify-center items-start w-full max-w-6xl mx-auto p-4">
      <div className="flex flex-col justify-center items-start w-full  rounded-md p-4">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="w-auto ">
          ...
          {putError && (
            <p className="text-red-500 mb-2">데이터를 업데이트하는 데 실패했습니다.</p>
          )}
          {isPutting && <p className="mb-2">업데이트 중...</p>}
          {responseData && (
            <p className="text-green-500 mb-2">데이터가 성공적으로 업데이트되었습니다.</p>
          )}
